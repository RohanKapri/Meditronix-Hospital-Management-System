...
<style>
    ...
    .container-xxl {
        ...
        background-image: url('https://cdn.pixabay.com/photo/2015/02/26/15/40/doctor-650534_1280.jpg');
    ...
    .doctor-social-links a:hover::after {
        opacity: 1;
        visibility: visible;
        bottom: -40px;
    }
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.doctor-card', function() {
            ...
            const doctorName = $(this).find('.doctor-name-id').text().trim();
            const doctorId = $(this).find('.doctor-id-display').text().trim();
            popupMessage.find('br').first().remove();
            popupMessage.find('center').text(`✨ ${doctorName} (${doctorId})'s Records ✨`);
