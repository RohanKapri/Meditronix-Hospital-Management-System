...
if (mysqli_connect_errno()) {
    ...
    echo "<div style='text-align: center; padding: 50px; background: #fff; border-radius: 15px; box-shadow: 0 0 20px rgba(0,0,0,0.1); margin-top: 100px;'>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($patient_id > 0 && !empty($patient_name) && !empty($message) && $rating >= 1 && $rating <= 5) {
        $created_at = date('Y-m-d H:i:s');
        $insert_query = "INSERT INTO `feedback` (`patient_id`, `patients' _name`, `message`, `rating`, `status`, `created_at`) 
                         VALUES ('$patient_id', '$patient_name', '$message', '$rating', '$status', '$created_at')";
        ...
        error_log("Invalid data: Patient ID: $patient_id, Name: $patient_name, Message: $message, Rating: $rating");
    }
}

$feedbackResult = mysqli_query($con, "SELECT `id`, `patient_id`, `patients' _name`, `message`, `rating`, `status`, `created_at` FROM `feedback` ORDER BY `created_at` DESC");
$feedbackData = [];
if ($feedbackResult) {
    ...
    error_log('Select Error: ' . mysqli_error($con));
}
...
<div class="container-wrapper">
    <style>
        ...
        :root {
            ...
            /* Body Background: Light Mix Diamond Color (subtle, shimmering light grey/white with hints of other light colors) */
            --body-bg-1: #F8F8F8; /* Very light grey */
            --body-bg-2: #F0F0F0; /* Slightly darker light grey */
            --body-bg-3: #E8F0F8; /* Light blue-grey */
            --body-bg-4: #F8F0E8; /* Light yellow-grey */
            --body-bg-5: #F0E8F8; /* Light violet-grey */
            --body-bg-6: #E8F8F0; /* Light green-grey */
        ...
        * {
        ...
        /* Body Styling: Attractive Light Rainbow Background */
        body {
        ...
        .slider-card {
            ...
            z-index: 5;
            background-image: url('https://cdn.pixabay.com/photo/2020/11/03/15/31/doctor-5710153_1280.jpg'); /* Placeholder image for card background */
        ...
        /* Add New Feedback Button Styling (Outside Carousel) */
        .btn-add-new {
    ...
    <div class="page-header">
        <h1><i class="fas fa-comments" style="margin-right: 20px; color: #6dd5ed;"></i>Meditronix: Patient Feedback Portal<i class="fas fa-star-half-alt" style="margin-left: 20px; color: #ff7e5f;"></i></h1>
        <p>Your voice matters! This portal showcases valuable feedback from our patients, helping us continuously improve our services. We appreciate your honest opinions and ratings, which guide us in delivering exceptional healthcare experiences. Explore what our patients are saying and see how we're striving for excellence.</p>
    </div>

    <div class="slider-container" id="sliderContainer">
        <div class="slider-track" id="sliderTrack">
            <?php
            // Function to get a relevant icon based on rating or status
            function getFeedbackIcon($rating, $status) {
                $status = strtolower($status);
                if ($rating >= 4) {
                    return 'fas fa-star'; // High rating
                } elseif ($rating == 3) {
                    return 'fas fa-star-half-alt'; // Medium rating
                } elseif ($rating < 3) {
                    return 'fas fa-frown'; // Low rating
                }
                ...
                if (strpos($status, 'active') !== false) {
                    ...
                    return 'fas fa-comment-alt'; // Default icon
                }
            }

            if (!empty($feedbackData)):
                foreach ($feedbackData as $row):
                    $iconClass = getFeedbackIcon($row['rating'], $row['status']);
            ?>
                    <div class="slider-card">
                    <div class="icon-container"><i class="<?php echo $iconClass; ?>"></i></div>
<h2>Feedback ID: <strong><?php echo htmlspecialchars($row['id']); ?></strong></h2>
<p><strong>Patient ID:</strong> <?php echo htmlspecialchars($row['patient_id']); ?></p>
<p><strong>Patient Name:</strong> <?php echo htmlspecialchars($row["patients' _name"]); ?></p>
...
<p><strong>Rating:</strong> <?php echo htmlspecialchars($row['rating']); ?></p>
                            ...
                            for ($i = 0; $i < 5; $i++) {
                                if ($i < $row['rating']) {
                                    echo '<i class="fas fa-star" style="color: #FFD700;"></i>'; // Gold star
                                } else {
                        ...
                        <p><strong>Submitted On:</strong> <?php echo date('d M Y', strtotime($row['created_at'])); ?></p>
                        <footer class="crud-buttons">
                            <!-- Buttons for actions on feedback -->
                            <a href='views_feedback.php?id=<?php echo $row['id']; ?>' class='btn btn-view' title='View Details'>👁️ View</a>
                            <button class='btn btn-resolve' data-feedback-id='<?php echo $row['id']; ?>' title='Mark as Reviewed'>✅ Mark Reviewed</button>
            ...
            ?>
                <div class="slider-card" style="flex: 0 0 100%; text-align: center; padding: 50px; background-image: url('https://placehold.co/700x400/F8F9FA/6C757D?text=No+Feedback+Available');">
                    <div class="icon-container"><i class="fas fa-exclamation-triangle"></i></div>
                    <h2>No Feedback Available</h2>
                    <p>Currently, there is no patient feedback to display. Encourage patients to submit their valuable opinions.</p>
                    <div class="text-center">
                        <a href="feedback_add.php" class="btn btn-add-new">➕ Submit New Feedback</a>
                    </div>
                </div>
            <?php
            endif;
    ...
    <div id="popup-message">✨ Thank You for Your Feedback! ✨
        <br>
        <center>✨MEDITRONIX APPRECIATES YOU✨</center>
    </div>
    <canvas id="fireworkCanvas"></canvas>

    <div class="modal-overlay" id="actionConfirmationModal">
        <div class="modal-content">
            <h3>Are you sure you want to mark this feedback as reviewed?</h3>
            <div class="modal-buttons">
                <button class="btn btn-yes" id="confirmActionYes">Yes, Mark Reviewed</button>
                <button class="btn btn-no" id="confirmActionNo">No, Cancel</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Custom Action Confirmation Modal Logic (for "Mark Reviewed")
        ...
        confirmActionYes.addEventListener('click', function() {
            if (feedbackIdToUpdate) {
                // In a real application, you would send an AJAX request here to update the status in the database
                console.log('Simulating mark as reviewed for feedback ID:', feedbackIdToUpdate);
                const cardToUpdate = document.querySelector(`.slider-card [data-feedback-id="${feedbackIdToUpdate}"]`).closest('.slider-card');
                if (cardToUpdate) {
                    const statusSpan = cardToUpdate.querySelector('p strong + span'); // Find the status span
                    if (statusSpan) {
                        statusSpan.textContent = 'Reviewed';
                        statusSpan.style.color = '#28a745'; // Green for reviewed
                    }
                }
                // In a real application, you might redirect or refresh the data after a successful update
                // window.location.href = 'feedback_update_status.php?id=' + feedbackIdToUpdate + '&status=reviewed';
            }
            actionConfirmationModal.classList.remove('show');
        });

        confirmActionNo.addEventListener('click', function() {
            feedbackIdToUpdate = null;
            actionConfirmationModal.classList.remove('show');
        });
