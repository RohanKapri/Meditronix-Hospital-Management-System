...
// Fetch news data
// Ensure all required fields are selected, including 'status' as per the latest request
...
<div class="container-wrapper">
    <style>
        ...
        :root {
            ...
            /* Mixed Light Blue-Greyish Rainbow Colors for Body Background with Wind Effect */
            --hazy-rainbow-bg-1: hsla(200, 18%, 85%, 0.6); /* Light Blue-Grey */
            --hazy-rainbow-bg-2: hsla(170, 18%, 85%, 0.6); /* Light Cyan-Grey */
            --hazy-rainbow-bg-3: hsla(120, 18%, 85%, 0.6); /* Light Green-Grey */
            --hazy-rainbow-bg-4: hsla(60, 18%, 85%, 0.6);  /* Light Yellow-Grey */
            --hazy-rainbow-bg-5: hsla(30, 18%, 85%, 0.6);  /* Light Orange-Grey */
            --hazy-rainbow-bg-6: hsla(300, 18%, 85%, 0.6); /* Light Purple-Grey */
            --hazy-rainbow-bg-7: hsla(240, 18%, 85%, 0.6); /* Light Blue-Violet-Grey */

            /* Light Violet Color for Container Background */
            --container-bg-gradient: linear-gradient(135deg, #E6E6FA, #D8BFD8); /* Lavender to Thistle */
        ...
        * {
        ...
        /* Body Styling: Mixed Light Blue-Greyish Rainbow Background with Wind Effect */
        body {
            background: linear-gradient(135deg,
                var(--hazy-rainbow-bg-1) 0%,
                var(--hazy-rainbow-bg-2) 14%,
                var(--hazy-rainbow-bg-3) 28%,
                var(--hazy-rainbow-bg-4) 42%,
                var(--hazy-rainbow-bg-5) 56%,
                var(--hazy-rainbow-bg-6) 70%,
                var(--hazy-rainbow-bg-7) 84%,
                var(--hazy-rainbow-bg-1) 100%
            );
            background-size: 1800% 1800%; /* Larger size for more subtle and flowing gradient */
            animation: gradientBackground 180s ease infinite alternate, pulseBackground 220s linear infinite; /* Slower, more ethereal animations */
            ...
            filter: saturate(1.04) contrast(1.04); /* Subtle filter for haziness */
        ...
        body::before {
            ...
            height: 100%;
            pointer-events: none;
            background: radial-gradient(circle at 15% 25%, rgba(255,255,255,0.03) 1px, transparent 1px), /* Fainter particles */
                        radial-gradient(circle at 85% 75%, rgba(255,255,255,0.03) 1px, transparent 1px),
                        radial-gradient(circle at 50% 50%, rgba(255,255,255,0.005) 1px, transparent 1px);
            background-size: 100px 100px, 110px 110px, 90px 90px; /* Larger, more spaced particles */
            animation: windEffect 400s linear infinite alternate, subtleZoom 450s linear infinite; /* Slower, more pronounced wind effect */
            z-index: -1;
        ...
        @keyframes subtleZoom {
            0% { background-size: 100px 100px, 110px 110px, 90px 90px; }
            50% { background-size: 103px 103px, 114px 114px, 93px 93px; }
            100% { background-size: 100px 100px, 110px 110px, 90px 90px; }
        }
        ...
        .container-wrapper {
            ...
            margin: 80px auto;
            background: var(--container-bg-gradient); /* Light violet gradient for the container */
        ...
        .slider-card {
            ...
            z-index: 5;
            background-image: url('https://cdn.pixabay.com/photo/2024/08/29/12/10/doctor-9006731_1280.jpg'); /* Placeholder image for card background */
        ...
        /* Add New News Article Button Styling (Outside Carousel) */
        .btn-add-new {
    ...
    <div class="page-header">
        <h1><i class="fas fa-newspaper" style="margin-right: 20px; color: #6dd5ed;"></i>Meditronix: Latest Medical News<i class="fas fa-bullhorn" style="margin-left: 20px; color: #ff7e5f;"></i></h1>
        <p>Stay updated with the most recent developments in healthcare and medical science. Our news portal brings you curated articles, breakthroughs, and important announcements to keep you informed and empowered. Explore the forefront of medical innovation with Meditronix.</p>
    </div>

    <div class="slider-container" id="sliderContainer">
        <div class="slider-track" id="sliderTrack">
            <?php
            // Function to get a relevant icon based on news content or status
            function getNewsIcon($title, $content, $status) {
                $title = strtolower($title);
                $content = strtolower($content);
                $status = strtolower($status);

                if (strpos($title, 'covid') !== false || strpos($content, 'pandemic') !== false) {
                    return 'fas fa-virus';
                } elseif (strpos($title, 'vaccine') !== false || strpos($content, 'immunization') !== false) {
                    return 'fas fa-syringe';
                } elseif (strpos($title, 'research') !== false || strpos($content, 'study') !== false) {
                    return 'fas fa-flask';
                } elseif (strpos($title, 'breakthrough') !== false || strpos($content, 'discovery') !== false) {
                    return 'fas fa-lightbulb';
                } elseif (strpos($title, 'health tips') !== false || strpos($content, 'wellness') !== false) {
                    return 'fas fa-heart';
                } elseif (strpos($status, 'published') !== false || strpos($status, 'active') !== false) {
                    return 'fas fa-check-circle';
                } elseif (strpos($status, 'draft') !== false || strpos($status, 'pending') !== false) {
            ...
            if (!empty($newsData)):
                foreach ($newsData as $row):
                    $iconClass = getNewsIcon($row['title'], $row['content'], $row['status']);
            ?>
                    <div class="slider-card">
                        ...
                        <p><strong>Content:</strong> <?php echo nl2br(htmlspecialchars($row['content'])); ?></p>
                        <p><strong>Status:</strong> <span style="font-weight: bold; color: <?php
                            $status_color = 'var(--text-color-dark)';
                            if (strtolower($row['status']) == 'published') { $status_color = '#28a745'; } // Green
                            elseif (strtolower($row['status']) == 'draft') { $status_color = '#ffc107'; } // Yellow/Orange
                            elseif (strtolower($row['status']) == 'archived') { $status_color = '#6c757d'; } // Grey
                            echo $status_color;
                        ...
                        <footer class="crud-buttons">
                            <a href='news_edit.php?id=<?php echo $row['id']; ?>' class='btn btn-edit' title='Edit'>✏️ Edit</a>
                            <button class='btn btn-delete' data-news-id='<?php echo $row['id']; ?>' title='Delete'>🗑️ Delete</button>
            ...
            ?>
                <div class="slider-card" style="flex: 0 0 100%; text-align: center; padding: 50px; background-image: url('https://placehold.co/700x400/F8F9FA/6C757D?text=No+News+Articles+Available');">
                    <div class="icon-container"><i class="fas fa-exclamation-triangle"></i></div>
                    <h2>No News Articles Available</h2>
                    <p>Currently, there are no news articles to display. Please check back later for updates or add a new article.</p>
                    <div class="text-center">
                        <a href="news_add.php" class="btn btn-add-new">➕ Add New Article</a>
                    </div>
                </div>
            <?php
            endif;
    ...
    <div id="popup-message">✨ Breaking Medical News ✨
        <br>
    ...
    <div class="modal-overlay" id="deleteConfirmationModal">
        <div class="modal-content">
            <h3>Are you sure you want to delete this news article?</h3>
    ...
    <script>
        ...
        document.querySelectorAll('.btn-delete').forEach(button => {
            button.addEventListener('click', function(event) {
                ...
                newsIdToDelete = this.dataset.newsId;
        ...
        confirmDeleteYes.addEventListener('click', function() {
            if (newsIdToDelete) {
                ...
                console.log('Simulating delete for news ID:', newsIdToDelete);
                const cardToRemove = document.querySelector(`.slider-card [data-news-id="${newsIdToDelete}"]`).closest('.slider-card');
                if (cardToRemove) {
                    cardToRemove.remove();
                }
                // Redirect to a delete script in a real scenario
                // window.location.href = 'news_delete.php?id=' + newsIdToDelete;
            }
