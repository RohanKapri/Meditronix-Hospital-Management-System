...
$contactQueriesResult = mysqli_query($con, "SELECT `id`, `name`, `email`, `subject`, `message`, `status`, `created_at` FROM `contact_queries:` ORDER BY `created_at` DESC");
$contactQueriesData = [];
if ($contactQueriesResult) {
    while ($row = mysqli_fetch_assoc($contactQueriesResult)) {
        $contactQueriesData[] = $row;
    }
} else {
    error_log("Error fetching contact queries data: " . mysqli_error($con));
    $contactQueriesData = [];
}
mysqli_close($con);
?>
...
<div class="container-wrapper">
    <style>
        ...
        :root {
            ...
            /* Attractive Light Rainbow Background with Crystal Diamond and Gold */
            --body-bg-1: #FFDAB9; /* Peach Puff - light orange */
            --body-bg-2: #E0FFFF; /* Light Cyan - light blue */
            --body-bg-3: #F0FFF0; /* Honeydew - light green */
            --body-bg-4: #FFFACD; /* Lemon Chiffon - light yellow */
            --body-bg-5: #E6E6FA; /* Lavender - light violet */
            --body-bg-6: #ADD8E6; /* Light Blue */

            /* Light Crystal Diamond and Gold for Container Background */
            --container-bg-gradient: linear-gradient(135deg, #F0F8FF, #FDF5E6); /* AliceBlue to OldLace (Crystal Diamond & Gold feel) */
        ...
        * {
        ...
        .slider-card {
            ...
            z-index: 5;
            background-image: url('https://cdn.pixabay.com/photo/2025/03/24/15/10/ai-generated-9490956_1280.png'); /* Placeholder image for card background */
        ...
        /* Card Title (Contact Query ID) Styling */
        .slider-card h2 {
            font-size: 2.5rem;
            color: var(--text-color-dark);
            margin-bottom: 20px;
        ...
        /* Paragraph Styling for Contact Query Details */
        .slider-card p {
            color: var(--text-color-dark);
            font-size: 1.3rem;
            margin-bottom: 15px;
        ...
        /* Add New Contact Query Button Styling (Outside Carousel) */
        .btn-add-new {
    ...
    <div class="page-header">
        <h1><i class="fas fa-headset" style="margin-right: 20px; color: #6dd5ed;"></i>Meditronix: Your Inquiries, Our Priority<i class="fas fa-envelope-open-text" style="margin-left: 20px; color: #ff7e5f;"></i></h1>
        <p>We value your feedback and are here to assist you. This portal provides a comprehensive overview of all user-submitted contact queries. Our dedicated team reviews each inquiry to ensure timely and effective responses, addressing your concerns with the utmost care and professionalism. Your satisfaction is our commitment.</p>
    </div>

    <div class="slider-container" id="sliderContainer">
        <div class="slider-track" id="sliderTrack">
            <?php
            // Function to get a relevant icon based on query subject or status
            function getContactQueryIcon($subject, $status) {
                $subject = strtolower($subject);
                $status = strtolower($status);

                if (strpos($subject, 'appointment') !== false || strpos($subject, 'scheduling') !== false) {
                    return 'fas fa-calendar-alt';
                } elseif (strpos($subject, 'billing') !== false || strpos($subject, 'payment') !== false) {
                    return 'fas fa-file-invoice-dollar';
                } elseif (strpos($subject, 'service') !== false || strpos($subject, 'treatment') !== false) {
                    return 'fas fa-hand-holding-medical';
                } elseif (strpos($subject, 'feedback') !== false || strpos($subject, 'suggestion') !== false) {
                    return 'fas fa-comment-dots';
                } elseif (strpos($subject, 'technical') !== false || strpos($subject, 'issue') !== false) {
                    return 'fas fa-bug';
                } elseif (strpos($status, 'resolved') !== false || strpos($status, 'closed') !== false) {
                    return 'fas fa-check-circle';
                } elseif (strpos($status, 'pending') !== false || strpos($status, 'new') !== false) {
            ...
            if (!empty($contactQueriesData)):
                foreach ($contactQueriesData as $row):
                    $iconClass = getContactQueryIcon($row['subject'], $row['status']);
            ?>
                    <div class="slider-card">
                        <div class="icon-container"><i class="<?php echo $iconClass; ?>"></i></div>
                        <h2>Query ID: <strong><?php echo htmlspecialchars($row['id']); ?></strong></h2>
                        ...
                        <p><strong>Status:</strong> <span style="font-weight: bold; color: <?php
                            ...
                            if (strtolower($row['status']) == 'resolved') { $status_color = '#28a745'; } // Green
                            elseif (strtolower($row['status']) == 'pending') { $status_color = '#ffc107'; } // Yellow/Orange
                            elseif (strtolower($row['status']) == 'closed') { $status_color = '#6c757d'; } // Grey
                            echo $status_color;
                        ...
                        <footer class="crud-buttons">
                            <!-- Buttons for actions on contact queries -->
            ...
            ?>
                <div class="slider-card" style="flex: 0 0 100%; text-align: center; padding: 50px; background-image: url('https://placehold.co/700x400/F8F9FA/6C757D?text=No+Contact+Queries+Available');">
                    <div class="icon-container"><i class="fas fa-exclamation-triangle"></i></div>
                    <h2>No Contact Queries Available</h2>
                    <p>Currently, there are no user inquiries to display. Encourage visitors to use the contact form.</p>
                    <div class="text-center">
                        <a href="contact_form.php" class="btn btn-add-new">✉️ Submit New Inquiry</a>
                    </div>
                </div>
            <?php
            endif;
    ...
    <script>
        ...
        let queryIdToResolve = null; // Renamed for clarity

        document.querySelectorAll('.btn-delete').forEach(button => {
            button.addEventListener('click', function(event) {
                event.preventDefault(); // Prevent default link behavior
                queryIdToResolve = this.dataset.queryId;
        ...
        confirmDeleteYes.addEventListener('click', function() {
            if (queryIdToResolve) {
                // In a real application, you would send an AJAX request here to update the status
                // For this example, we'll simulate the resolution by changing the card's status display
                console.log('Simulating resolution for query ID:', queryIdToResolve);
                const cardToUpdate = document.querySelector(`.slider-card [data-query-id="${queryIdToResolve}"]`).closest('.slider-card');
                if (cardToUpdate) {
                    const statusSpan = cardToUpdate.querySelector('p strong + span'); // Find the status span
                    if (statusSpan) {
                        statusSpan.textContent = 'Resolved';
                        statusSpan.style.color = '#28a745'; // Green for resolved
                    }
                }
                // In a real application, you might redirect or refresh the data
                // window.location.href = 'contact_query_resolve.php?id=' + queryIdToResolve;
            }
            deleteConfirmationModal.classList.remove('show');
        });

        confirmDeleteNo.addEventListener('click', function() {
            queryIdToResolve = null;
            deleteConfirmationModal.classList.remove('show');
        });
