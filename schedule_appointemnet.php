...
if (mysqli_connect_errno()) {
    ...
    echo "<p style='color: #6c757d; font-size: 1.2rem;'>Once fixed, appointment entries will appear here automatically.</p>";
    echo "</div>";
...
// Handle Approve/Decline actions (simulated for demonstration)
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action === 'approve' || $action === 'decline') {
        ...
        // UPDATE Query: Update the status of an appointment
        $update_query = "UPDATE `appointments` SET `status` = '$new_status' WHERE `id` = $appointment_id";
        if (mysqli_query($con, $update_query)) {
            // Redirect to refresh the page and show updated status, or handle via AJAX in a real app
            header("Location: " . $_SERVER['PHP_SELF']);
            ob_end_flush();
            exit();
        } else {
            error_log("Error updating appointment status: " . mysqli_error($con));
        }
    }
}

// Fetch appointments data for display in the carousel
// SELECT Query: Retrieve all necessary columns from the 'appointments' table.
// Note: Corrected table name from `appointments:` to `appointments` and enclosed in backticks for safety.
// Also, `doctor's_name` is enclosed in backticks due to the apostrophe.
$appointmentsResult = mysqli_query($con, "SELECT `id`, `patient_id`, `patient_name`, `doctor_id`, `doctor's_name`, `service_id`, `appointment_date`, `appointment_time`, `status` FROM `appointments:` ORDER BY `appointment_date` DESC, `appointment_time` DESC");
$appointmentsData = [];
...
// SELECT * FROM `appointments` WHERE 1;

// Specific SELECT Query (as provided by user, similar to current fetch)
// SELECT `id`, `patient_id`, `patient_name`, `doctor_id`, `doctor's_name`, `service_id`, `appointment_date`, `appointment_time`, `status` FROM `appointments` WHERE 1;

// INSERT Query Template (as provided by user - adjusted for correct table name and column names)
// INSERT INTO `appointments`(`id`, `patient_id`, `patient_name`, `doctor_id`, `doctor's_name`, `service_id`, `appointment_date`, `appointment_time`, `status`) VALUES ('[value-1]','[value-2]','[value-3]','[value-4]','[value-5]','[value-6]','[value-7]','[value-8]','[value-9]')

// UPDATE Query Template (as provided by user - adjusted for correct table name and column names)
// UPDATE `appointments`
// SET `id`='[value-1]',
//     `patient_id`='[value-2]',
//     `patient_name`='[value-3]',
//     `doctor_id`='[value-4]',
//     `doctor's_name`='[value-5]',
//     `service_id`='[value-6]',
//     `appointment_date`='[value-7]',
//     `appointment_time`='[value-8]',
//     `status`='[value-9]'
// WHERE 1;
*/
?>
...
<div class="container-wrapper">
    <style>
        ...
        :root {
            ...
            /* Container Background: Bloosim Rose Color with Diamond Crystal Effect */
        ...
        * {
        ...
        .slider-card {
            ...
            z-index: 5;
            background-image: url('https://cdn.pixabay.com/photo/2017/08/22/13/17/doctor-2668918_1280.png'); /* Placeholder image for card background */
        ...
        /* Card Title (Appointment ID) Styling */
        .slider-card h2 {
            font-size: 2.5rem;
            color: var(--text-color-dark);
            margin-bottom: 20px;
        ...
        /* Action Buttons Styling (Approve/Decline) */
        .action-buttons {
            display: flex;
            justify-content: center;
            gap: 15px;
            width: 100%;
            margin-bottom: 20px; /* Space between buttons and social links */
        }
        .action-buttons .btn {
            padding: 12px 25px;
            border-radius: 10px;
        ...
        /* Custom Modal for Action Confirmation */
        .modal-overlay {
            position: fixed;
            top: 0;
            left: 0;
    ...
    <div class="page-header">
        <h1><i class="fas fa-calendar-alt" style="margin-right: 20px; color: #6dd5ed;"></i>Meditronix: Appointment Availability<i class="fas fa-check-double" style="margin-left: 20px; color: #feb47b;"></i></h1>
        <p>Efficiently manage your patient appointments with our intuitive availability dashboard. Review, approve, or decline consultations with ease, ensuring a streamlined workflow and optimal patient care. Your schedule, simplified and at your fingertips.</p>
    </div>
    ...
    <div class="slider-container" id="sliderContainer">
        <div class="slider-track" id="sliderTrack">
            ...
            function getAppointmentIcon($status, $service_id = null) {
                $status = strtolower($status);
                if ($status == 'approved') {
                    return 'fas fa-check-circle';
                } elseif ($status == 'declined') {
                    return 'fas fa-times-circle';
                } elseif ($status == 'pending' || $status == 'scheduled') { // Treat 'scheduled' as pending for icon purposes
                    return 'fas fa-hourglass-half';
                } elseif ($status == 'completed') {
                    return 'fas fa-calendar-check';
                }

                // Fallback based on service type if available (dummy logic)
                if ($service_id == 1) { // Example: General Check-up
                    return 'fas fa-notes-medical';
                } elseif ($service_id == 2) { // Example: Specialist Consultation
                    return 'fas fa-user-md';
                }
                return 'fas fa-calendar-day'; // Default icon
            }

            if (!empty($appointmentsData)):
                foreach ($appointmentsData as $row):
                    $iconClass = getAppointmentIcon($row['status'], $row['service_id']);
            ?>
                    <div class="slider-card">
                        <div class="icon-container"><i class="<?php echo $iconClass; ?>"></i></div>
                        <h2>Appointment ID: <strong><?php echo htmlspecialchars($row['id']); ?></strong></h2>
                        ...
                        <p><strong>Doctor Name:</strong> <strong><?php echo htmlspecialchars($row["doctor's_name"]); ?></strong></p>
                        ...
                        <p><strong>Status:</strong> <span style="font-weight: bold; color: <?php
                            ...
                            if (strtolower($row['status']) == 'approved') { $status_color = '#28a745'; } // Green
                            elseif (strtolower($row['status']) == 'pending' || strtolower($row['status']) == 'scheduled') { $status_color = '#ffc107'; } // Orange
                            elseif (strtolower($row['status']) == 'declined') { $status_color = '#dc3545'; } // Red
                            elseif (strtolower($row['status']) == 'completed') { $status_color = '#17a2b8'; } // Cyan
                            echo $status_color;
                        ?>;"><?php echo htmlspecialchars(ucfirst($row['status'])); ?></span></p>
                        <footer class="action-buttons">
                            <!-- Only Approve and Decline Buttons are shown as per request -->
                          <a href='approved_appointment.php?action=approve&id=<?php echo $row['id']; ?>' class='btn btn-approve' title='Approve Appointment'>✅ Approve</a>
                          <a href='decline_appointement.php?action=decline&id=<?php echo $row['id']; ?>' class='btn btn-decline' title='Decline Appointment'>❌ Decline</a>
            ...
            ?>
                <div class="slider-card" style="flex: 0 0 100%; text-align: center; padding: 50px; background-image: url('https://placehold.co/700x400/F8F9FA/6C757D?text=No+Appointments+Available');">
                    ...
                    <p>Currently, there are no scheduled appointments requiring your approval or decline. All new appointments will appear here automatically.</p>
                    <div class="text-center">
                        <a href="schedule_appointment.php" class="btn btn-add-new">➕ Schedule New Appointment</a>
                    </div>
                </div>
            <?php
            endif;
    ...
    <div id="popup-message">✨ Manage Appointments with Ease ✨
        <br>
        <center>✨MEDITRONIX AVAILABILITY✨</center>
    </div>
    ...
    <script>
        ...
        document.querySelectorAll('.slider-card').forEach(card => {
            card.addEventListener('click', function(event) {
                ...
                if (!event.target.closest('.action-buttons')) { // Ensure click is not on action buttons
        ...
        // Custom Action Confirmation Modal Logic (for Approve/Decline buttons)
        ...
        document.querySelectorAll('.btn-approve, .btn-decline').forEach(button => {
            button.addEventListener('click', function(event) {
                event.preventDefault(); // Prevent default button behavior
                pendingActionUrl = this.href; // Store the URL for the action
                actionConfirmationModal.classList.add('show');
            });
        });

        confirmActionYes.addEventListener('click', function() {
            if (pendingActionUrl) {
                window.location.href = pendingActionUrl; // Redirect to perform the action
            }
            actionConfirmationModal.classList.remove('show');
            pendingActionUrl = null;
        });

        confirmActionNo.addEventListener('click', function() {
            pendingActionUrl = null;
            actionConfirmationModal.classList.remove('show');
        });

        actionConfirmationModal.addEventListener('click', function(event) {
