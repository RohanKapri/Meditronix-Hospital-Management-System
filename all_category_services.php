...
$servicesResult = mysqli_query($con, "SELECT `id`, `doctor's_name`, `name`, `description`, `fee`, `created_at` FROM `services` ORDER BY `created_at` DESC");
$servicesData = [];
if ($servicesResult) {
    ...
    error_log("Error fetching services data: " . mysqli_error($con));
    $servicesData = [];
}
mysqli_close($con);
?>
...
<div class="container-wrapper">
    <style>
        ...
        :root {
            ...
            --hazy-rainbow-1: hsla(200, 20%, 90%, 0.7); /* Light Blue-Grey */
            --hazy-rainbow-2: hsla(150, 20%, 90%, 0.7); /* Light Green-Grey */
            --hazy-rainbow-3: hsla(60, 20%, 90%, 0.7);  /* Light Yellow-Grey */
            --hazy-rainbow-4: hsla(30, 20%, 90%, 0.7);  /* Light Orange-Grey */
            --hazy-rainbow-5: hsla(330, 20%, 90%, 0.7); /* Light Pink-Grey */
            --hazy-rainbow-6: hsla(270, 20%, 90%, 0.7); /* Light Purple-Grey */

            /* Light Gradient Pink for Container Background */
            --light-gradient-pink: linear-gradient(135deg, #FFD1DC, #FFB6C1); /* Soft Pink to Lighter Pink */

            --text-color-dark: #2c3e50;
            --text-color-medium: #555;
            --text-color-light: #888;
            --card-bg: rgba(255, 255, 255, 0.99); /* Slightly transparent white for cards */
        ...
        * {
        ...
        /* Body Styling: Hazy Rainbow Background with Wind Effect */
        body {
            background: linear-gradient(135deg,
                var(--hazy-rainbow-1) 0%,
                var(--hazy-rainbow-2) 16%,
                var(--hazy-rainbow-3) 33%,
                var(--hazy-rainbow-4) 50%,
                var(--hazy-rainbow-5) 66%,
                var(--hazy-rainbow-6) 83%,
                var(--hazy-rainbow-1) 100%
            );
            background-size: 1200% 1200%; /* Larger size for more subtle and flowing gradient */
            animation: gradientBackground 120s ease infinite alternate, pulseBackground 180s linear infinite; /* Slower, more ethereal animations */
            ...
            filter: saturate(1.02) contrast(1.02); /* Subtle filter for haziness */
        ...
        body::before {
            ...
            height: 100%;
            pointer-events: none;
            background: radial-gradient(circle at 15% 25%, rgba(255,255,255,0.05) 1px, transparent 1px), /* Fainter particles */
                        radial-gradient(circle at 85% 75%, rgba(255,255,255,0.05) 1px, transparent 1px),
                        radial-gradient(circle at 50% 50%, rgba(255,255,255,0.02) 1px, transparent 1px);
            background-size: 80px 80px, 90px 90px, 70px 70px; /* Larger, more spaced particles */
            animation: windEffect 300s linear infinite alternate, subtleZoom 350s linear infinite; /* Slower, more pronounced wind effect */
            z-index: -1;
        ...
        @keyframes subtleZoom {
            0% { background-size: 80px 80px, 90px 90px, 70px 70px; }
            50% { background-size: 83px 83px, 94px 94px, 73px 73px; }
            100% { background-size: 80px 80px, 90px 90px, 70px 70px; }
        }
        ...
        .container-wrapper {
            ...
            margin: 80px auto;
            background: var(--light-gradient-pink); /* Light gradient pink for the container */
        ...
        /* Keyframe Animation for Automatic Carousel Scroll */
        ...
        .slider-card {
            ...
            z-index: 5;
            background-image: url('https://cdn.pixabay.com/photo/2024/08/18/04/44/ai-generated-8976951_1280.png'); /* Placeholder image for card background */
        ...
        /* Paragraph Styling for Service Details */
        .slider-card p {
            color: var(--text-color-dark);
            font-size: 1.3rem;
            margin-bottom: 15px;
        ...
        /* Strong/Highlighted Text Styling */
        ...
        /* Add New Service Button Styling */
        .btn-add-new {
    ...
    <div class="page-header">
        <h1><i class="fas fa-hand-holding-medical" style="margin-right: 20px; color: #e52e71;"></i>Meditronix: Our Comprehensive Services<i class="fas fa-hospital-alt" style="margin-left: 20px; color: #3a7bd5;"></i></h1>
        <p>Explore the wide array of healthcare services offered by Meditronix, designed to cater to your diverse medical needs. From specialized treatments to general wellness programs, our services are delivered with precision, care, and the latest medical advancements. Discover how we can support your journey to optimal health and well-being.</p>
    </div>

    <div class="slider-container" id="sliderContainer">
        <div class="slider-track" id="sliderTrack">
            <?php
            // Function to get a relevant icon based on service name
            function getServiceIcon($serviceName) {
                $serviceName = strtolower($serviceName);
                if (strpos($serviceName, 'cardiac') !== false || strpos($serviceName, 'heart') !== false) {
                    return 'fas fa-heartbeat';
                } elseif (strpos($serviceName, 'neurology') !== false || strpos($serviceName, 'brain') !== false) {
                    return 'fas fa-brain';
                } elseif (strpos($serviceName, 'pediatric') !== false || strpos($serviceName, 'child') !== false) {
                    return 'fas fa-child';
                } elseif (strpos($serviceName, 'orthopedic') !== false || strpos($serviceName, 'bone') !== false) {
                    return 'fas fa-bone';
                } elseif (strpos($serviceName, 'dermatology') !== false || strpos($serviceName, 'skin') !== false) {
                    return 'fas fa-allergies';
                } elseif (strpos($serviceName, 'oncology') !== false || strpos($serviceName, 'cancer') !== false) {
                    return 'fas fa-dna';
                } elseif (strpos($serviceName, 'radiology') !== false || strpos($serviceName, 'x-ray') !== false || strpos($serviceName, 'imaging') !== false) {
                    return 'fas fa-x-ray';
                } elseif (strpos($serviceName, 'surgery') !== false || strpos($serviceName, 'surgical') !== false) {
                    return 'fas fa-cut';
                } elseif (strpos($serviceName, 'ophthalmology') !== false || strpos($serviceName, 'eye') !== false) {
                    return 'fas fa-eye';
                } elseif (strpos($serviceName, 'dental') !== false || strpos($serviceName, 'dentistry') !== false) {
                    return 'fas fa-tooth';
                } elseif (strpos($serviceName, 'psychiatry') !== false || strpos($serviceName, 'mental health') !== false) {
                    return 'fas fa-brain';
                } elseif (strpos($serviceName, 'gastroenterology') !== false || strpos($serviceName, 'digestive') !== false) {
                    return 'fas fa-stomach';
                } elseif (strpos($serviceName, 'endocrinology') !== false || strpos($serviceName, 'hormone') !== false) {
                    return 'fas fa-flask';
                } elseif (strpos($serviceName, 'urology') !== false || strpos($serviceName, 'kidney') !== false) {
                    return 'fas fa-kidneys';
                } elseif (strpos($serviceName, 'gynecology') !== false || strpos($serviceName, 'women') !== false) {
                    return 'fas fa-venus';
                } elseif (strpos($serviceName, 'anesthesia') !== false) {
                    return 'fas fa-syringe';
                } elseif (strpos($serviceName, 'emergency') !== false) {
                    return 'fas fa-ambulance';
                } elseif (strpos($serviceName, 'internal medicine') !== false) {
                    return 'fas fa-user-nurse';
                } elseif (strpos($serviceName, 'physiotherapy') !== false || strpos($serviceName, 'rehabilitation') !== false) {
                    return 'fas fa-wheelchair';
                } elseif (strpos($serviceName, 'pharmacy') !== false || strpos($serviceName, 'medication') !== false) {
                    return 'fas fa-pills';
                } elseif (strpos($serviceName, 'vaccination') !== false || strpos($serviceName, 'immunization') !== false) {
                    return 'fas fa-syringe';
                } elseif (strpos($serviceName, 'consultation') !== false) {
                    return 'fas fa-comments-medical';
                } else {
                    return 'fas fa-hospital-user'; // Default icon for general services
                }
            }

            if (!empty($servicesData)):
                foreach ($servicesData as $row):
                    $iconClass = getServiceIcon($row['name']);
            ?>
                    <div class="slider-card">
                        <div class="icon-container"><i class="<?php echo $iconClass; ?>"></i></div>
                        <h2><?php echo htmlspecialchars($row['name']); ?></h2>
                        <p><strong>Service ID:</strong> <?php echo htmlspecialchars($row['id']); ?></p>
                        <p><strong>Doctor:</strong> <?php echo htmlspecialchars($row["doctor's_name"]); ?></p>
                        ...
                        <p><strong>Added On:</strong> <?php echo date('d M Y', strtotime($row['created_at'])); ?></p>
                        <footer class="crud-buttons">
                            <a href='service_edit.php?id=<?php echo $row['id']; ?>' class='btn btn-edit' title='Edit'>✏️ Edit</a>
                            <button class='btn btn-delete' data-service-id='<?php echo $row['id']; ?>' title='Delete'>🗑️ Delete</button>
                            <div class="card-social-links">
                                <a href='https://www.linkedin.com/company/meditronix' target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
                                <a href='https://www.youtube.com/user/meditronix' target="_blank" title="YouTube"><i class="fab fa-youtube"></i></a>
                                <a href='https://en.wikipedia.org/wiki/Healthcare' target="_blank" title="Wikipedia"><i class="fab fa-wikipedia-w"></i></a>
            ...
            ?>
                <div class="slider-card" style="flex: 0 0 100%; text-align: center; padding: 50px; background-image: url('https://placehold.co/700x400/F8F9FA/6C757D?text=No+Services+Available');">
                    <div class="icon-container"><i class="fas fa-exclamation-triangle"></i></div>
                    <h2>No Services Available</h2>
                    <p>Currently, there are no service profiles to display. Please check back later for updates or add a new service.</p>
                    <div class="text-center">
                        <a href="service_add.php" class="btn btn-add-new">➕ Add New Service</a>
                    </div>
                </div>
            <?php
            endif;
...
<br>
   ...
   <footer style="text-align: center; padding: 20px 0; background-color: #f9f9f9; font-family: Arial, sans-serif; font-size: 14px; color: #333;">
    &copy; <?php echo date("Y"); ?> Meditronix. All rights reserved. Pioneering healthcare insights for a healthier tomorrow. Designed with 
    <span style="color: #e52e71;">&hearts;</span> for Rohan Kapri.
</footer>
    ...
    <script>
        ...
        let serviceIdToDelete = null; // Changed from doctorIdToDelete to serviceIdToDelete

        document.querySelectorAll('.btn-delete').forEach(button => {
            button.addEventListener('click', function(event) {
                event.preventDefault(); // Prevent default link behavior
                serviceIdToDelete = this.dataset.serviceId; // Changed to serviceId
        ...
        confirmDeleteYes.addEventListener('click', function() {
            if (serviceIdToDelete) {
                ...
                console.log('Simulating delete for service ID:', serviceIdToDelete);
                const cardToRemove = document.querySelector(`.slider-card [data-service-id="${serviceIdToDelete}"]`).closest('.slider-card');
                if (cardToRemove) {
                    cardToRemove.remove();
                }
                // Redirect to a delete script in a real scenario
                // window.location.href = 'service_delete.php?id=' + serviceIdToDelete;
            }
...
<!-- Dummy Queries as per your instruction, not executed here but written for clarity -->

<!-- SELECT -->
<!-- SELECT id, `doctor's_name`, name, description, fee, status, created_at FROM services WHERE 1 -->

<!-- INSERT -->
<!-- INSERT INTO services(id, `doctor's_name`, name, description, fee, status, created_at) 
VALUES ('[value-1]', '[value-2]', '[value-3]', '[value-4]', '[value-5]', '[value-6]', '[value-7]') -->

<!-- UPDATE -->
<!-- UPDATE services SET id='[value-1]', `doctor's_name`='[value-2]', name='[value-3]', description='[value-4]', fee='[value-5]', status='[value-6]', created_at='[value-7]' WHERE 1 -->
